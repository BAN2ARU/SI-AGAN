...
def get_outputs(inputs, skip=False):
    ...
    with tf.variable_scope("Model") as scope:
        ...
        rev_mask_a = 1-mask_a
        rev_mask_b = 1-mask_b
        rev_middle_a = tf.multiply(images_a, 1-mask_a)
        rev_middle_b = tf.multiply(images_b, 1-mask_b)
        ...
        fake_mask_a_on_a = tf.multiply(fake_images_a_from_g, mask_b)
        fake_mask_b_on_b = tf.multiply(fake_images_b_from_g, mask_a)
    ...
    return {
        ...
        'masked_ims': [mask_a_on_a, mask_b_on_b, mask_acycle_on_fakeA, mask_bcycle_on_fakeB, fake_mask_b_on_b, fake_mask_a_on_a],
        'masks': [mask_a, mask_b, mask_acycle, mask_bcycle, rev_mask_a, rev_mask_b, rev_middle_a, rev_middle_b],
...
def build_resnet_block(inputres, dim, name="resnet", padding="REFLECT"):
    ...
    with tf.variable_scope(name):
        out_res = tf.pad(inputres, [[0, 0], [1, 1], [
            1, 1], [0, 0]], padding)
        out_res = layers.separable_conv2d(
...
def discriminator(inputdisc,  mask, transition_rate, donorm,  name="discriminator"):
    ...
    with tf.variable_scope(name):
        ...
        o_c2 = layers.separable_conv2d(pad_o_c1, donorm, ndf * 2, f, f, 2, 2,
                                     0.02, "VALID", "c2",  relufactor=0.2)

        pad_o_c2 = tf.pad(o_c2, [[0, 0], [padw, padw], [
            padw, padw], [0, 0]], "CONSTANT")

        o_c3 = layers.separable_conv2d(pad_o_c2, donorm, ndf * 4, f, f, 2, 2,
