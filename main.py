...
class CycleGAN:
    ...
    def __init__(self, pool_size, lambda_a,
        ...
        self._size_before_crop_h = 224
        self._size_before_crop_w = 100
        self._switch = switch
    ...
    def compute_losses(self):
        ...
        *_summ -> Summary variables for above loss functions
        ...
        perceptual_loss_a = \
            self._lambda_a * 10 * losses.perceptual_loss(
                real_images=self.input_a, generated_images=self.cycle_images_a,
            )
        perceptual_loss_b = \
            self._lambda_b * 10 * losses.perceptual_loss(
                real_images=self.input_b, generated_images=self.cycle_images_b,
            )
        ...
        g_loss_A = \
            cycle_consistency_loss_a + cycle_consistency_loss_b + lsgan_loss_b + perceptual_loss_a
        g_loss_B = \
            cycle_consistency_loss_b + cycle_consistency_loss_a + lsgan_loss_a + perceptual_loss_b
    ...
    def train(self):
        ...
        self.inputs = data_loader.load_data(
            self._dataset_name, self._size_before_crop_h, self._size_before_crop_w,
    ...
    def test(self):
        ...
        img_i_path = './input/SYSU_F2/trainB'
        img_j_path = './input/SYSU_F2/trainA'

        img_i_list = os.listdir(img_i_path)
        img_j_list = os.listdir(img_j_path)

        total_data = len(img_i_list)
        ...
        with tf.Session() as sess:
            ...
            names = ['input_A', 'mask_A', 'masked_inputA', 'fakeB',
                     'input_B', 'mask_B', 'masked_inputB', 'fakeA']

            for i in range(0, total_data):
                img_i = Image.open(img_i_path + '/' + img_i_list[i])
                img_j = Image.open(img_j_path + '/' + img_j_list[i])

                img_i = np.array(img_i)
                img_j = np.array(img_j)

                img_i = np.subtract(tf.divide(img_i, 127.5), 1)
                img_j = np.subtract(tf.divide(img_j, 127.5), 1)

                img_i = np.expand_dims(img_i, axis=0)
                img_j = np.expand_dims(img_j, axis=0)

                print("Saving image {}/{}".format(i, total_data))
                fake_A_temp, fake_B_temp, masks, masked_ims = sess.run([
                    self.fake_images_a,
                    self.fake_images_b,
                    self.masks,
                ...
                tensors = [img_i, masks[0], masked_ims[0], fake_B_temp,
                           img_j, masks[1], masked_ims[1], fake_A_temp]

                for name, tensor in zip(names, tensors):
                    image_name = name + '_' + str(i) + ".jpg"
                    # image_name = name + str(n_i) + ".jpg"

                    if not os.path.isdir(os.path.join(self._images_dir, name)):
                        os.mkdir(os.path.join(self._images_dir, name))

                    if 'mask_' in name:
                        imsave(os.path.join(self._images_dir, name, image_name),
                               (np.squeeze(tensor[0]))
                               )
                    else:
...
def parse_args():
    desc = "Tensorflow implementation of cycleGan using 1"
    parser = argparse.ArgumentParser(description=desc)

    parser.add_argument('--to_train', type=int, default=1, help='Whether it is train or false.')
    parser.add_argument('--log_dir',
              type=str,
              default='./output/AGGAN/SYSU_AGGAN',
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./output/AGGAN/SYSU_finetuning/20210827-123334_switch0_thres_0.1', help='The name of the train/test split.')
